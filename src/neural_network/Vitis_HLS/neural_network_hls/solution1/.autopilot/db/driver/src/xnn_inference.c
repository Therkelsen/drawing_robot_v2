...
u32 XNn_inference_Get_out_r(XNn_inference *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XNn_inference_ReadReg(InstancePtr->Axi_cpu_BaseAddress, XNN_INFERENCE_AXI_CPU_ADDR_OUT_R_DATA);
    return Data;
}

u32 XNn_inference_Get_out_r_vld(XNn_inference *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XNn_inference_ReadReg(InstancePtr->Axi_cpu_BaseAddress, XNN_INFERENCE_AXI_CPU_ADDR_OUT_R_CTRL);
    return Data & 0x1;
}
